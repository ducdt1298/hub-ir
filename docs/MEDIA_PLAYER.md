...
For an entity added from the UI this lives under **Configure**, as a *Source
names* field taking the same mapping — `HDMI1: DVD Player`, `VGA: null`. It is
not on the create form, because the source list only becomes visible once the
device file has been loaded. Saving reloads the entity, so the renamed sources
