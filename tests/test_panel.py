...
async def test_nothing_learned_is_reported_as_a_failure(
    ...
    with pytest.raises(HomeAssistantError, match="No infrared code was received"):
        await async_learn_ir_code(hass, REMOTE_ENTITY_ID)
...
def test_the_panel_carries_its_own_text() -> None:
    """The panel had a translation table. Removing it has to stay removed.

    A lookup with one language behind it is indirection with nothing on the far
    side, and the English fallback at every call site meant an untranslated
    string looked correct to whoever added it. Both are gone; the markup holds
    its own text.
    """
    js = _panel_source()

    assert "const STRINGS" not in js
    assert "_lang(" not in js
    assert "function tr(" not in js
    assert "planText" not in js
    assert "this._t(" not in js


def test_the_shipped_integration_is_english_only() -> None:
    """One language, so every string is readable by whoever maintains it."""
    package = Path(__file__).resolve().parent.parent / "custom_components" / "hub_ir"
    offenders = [
        path.relative_to(package).as_posix()
        for path in package.rglob("*")
        # codes/ is created at runtime and holds downloaded device files.
        if path.is_file()
        and "codes" not in path.relative_to(package).parts
        and path.suffix in {".py", ".js", ".json", ".yaml"}
        and _VIETNAMESE.search(path.read_text(encoding="utf-8"))
    ]
